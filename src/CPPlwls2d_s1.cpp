...
Eigen::MatrixXd CPPlwls2d_s1( const double & bw, const std::string kernel_type, const Eigen::Map<Eigen::VectorXd> & win, 
                              const Eigen::Map<Eigen::MatrixXd> & tin, const Eigen::Map<Eigen::VectorXd> & yin, 
                              const Eigen::Map<Eigen::MatrixXd> & xin, const Eigen::Map<Eigen::MatrixXd> & zin,
  ...
  for (unsigned int i = 0; i != nUnknownPoints; ++i){
    ...
    if((p+1)*(m+1)+q >= indxSize){
      Rcpp::warning("Cannot do the estimation with less than (p+1)*(m+1)+q+1 points. tout id = %d", i);
      for (unsigned int y = 0; y != p+q+1; ++y) result(i, y) = std::numeric_limits<double>::quiet_NaN();
    } else {
      // make the design matrix
      Eigen::MatrixXd X(indxSize, (p+1)*(m+1)+q);
      X.setOnes();
      ...
      for (unsigned int y = (p+1)*(m+1); y != (p+1)*(m+1)+q; ++y){
        X.col(y) = lz.col(y-(p+1)*(m+1));
      }
